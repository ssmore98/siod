...
static int slave(const std::string & argv0, const bool & is_write, const bool & is_random, const uint16_t & iosize, 
        uint16_t & qdepth, const uint8_t & key, const std::string & logfile_prefix, const uint16_t & dno,
        const unsigned char * const iddata, const uint16_t & rank, const key_t & shmkey) {
    ...
    std::set<int> fds;
    {
            ...
            bzero(resp, sizeof(resp));
               switch (int retval = sg_ll_inquiry(fd, 0, 1, 0x80, resp, sizeof(resp), 0, 0)) {
                   ...
                   default:
                       {
                           std::ostringstream o;
                           o << "sg_ll_inquiry(0x" << std::setfill('0') << std::hex <<
                               std::setw(2) <<retval << ") : unknown error";
        ...
        if (logfp) gzprintf(logfp,  "%s %s %c %c %hu %hu %hu %s %s %s\n", LoglineStart().c_str(),
                argv0.c_str(), is_write ? 'W' : 'R', is_random ? 'R' : 'S', iosize, qdepth,
                key, logfile_prefix.c_str(), device.c_str(), sno.c_str());
    ...
    uint32_t blocksize = 0;
    {
        ...
        switch (int retval = sg_ll_readcap_16(ios[0].fd, 0, 0, resp, 32, 0, 0)) {
            ...
            default:
                {
                    std::ostringstream o;
                    o << "sg_ll_readcap_16(0x" << std::setfill('0') << std::hex << std::setw(2)
