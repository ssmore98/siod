...
typedef enum {ErrorNone, ErrorSyscall, ErrorIO, ErrorData, ErrorBlock,
    ErrorSignal, ErrorInternal} ErrorType;
...
static void logprint(const char * const file, const int & line,
...
static std::string buffer2str(const unsigned char * const buf,
        const uint16_t & sz) {
